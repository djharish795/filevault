...
export const FolderShareModal = ({ isOpen, onClose, folderName, folderId, projectId }: FolderShareModalProps) => {
  ...
  const handleAddUser = async (userObj: SearchUser | string) => {
    if (!folderId) return;
    setIsSaving(true);
    try {
      let targetEmail = typeof userObj === 'string' ? userObj : userObj.email;
      let targetUserId = typeof userObj === 'string' ? undefined : userObj.id;
      
      // If we only have an email, we might need the user ID since the folder access endpoint expects userId
      if (!targetUserId && typeof userObj === 'string') {
          // We can try to find the user from searchUsers
          const found = searchUsers.find(u => u.email === targetEmail);
          if (found) targetUserId = found.id;
          else {
              toast({ variant: 'destructive', title: 'Must select a valid user from the dropdown for folder sharing' });
              setIsSaving(false);
              return;
          }
      }

      const res = await apiClient.post(`/folders/${folderId}/access`, { userId: targetUserId });
      const newPerson = res.data.data.user;
      setPeople(prev => [...prev, newPerson]);
      setSearchUsers(prev => prev.filter(u => u.id !== targetUserId));
      setInputValue('');
      setShowDropdown(false);
      queryClient.invalidateQueries({ queryKey: ['projects'] });
      toast({ variant: 'success', title: 'Access granted', description: `${newPerson.name} can now access this folder.` });
    } catch (err: any) {
  ...
  const handleRemoveAccess = async (userId: string, name: string) => {
    if (!folderId) return;
    try {
      await apiClient.delete(`/folders/${folderId}/access/${userId}`);
      setPeople(prev => prev.filter(p => p.userId !== userId && (p as any).id !== userId));
      queryClient.invalidateQueries({ queryKey: ['projects'] });
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-lg bg-white dark:bg-slate-900 border dark:border-slate-800 p-0 overflow-hidden shadow-2xl rounded-xl">
        ...
        <DialogHeader className="px-6 py-4 border-b dark:border-slate-800">
          <DialogTitle className="text-xl font-medium truncate">Share '{folderName}'</DialogTitle>
          <DialogDescription className="text-sm text-slate-500 mt-0.5">
            People with access can view and upload files in this folder.
          </DialogDescription>
        </DialogHeader>
                ...
                <div className="absolute top-full left-0 right-0 mt-1 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg shadow-lg z-50 max-h-48 overflow-y-auto">
                  {suggestions.map(u => (
                    <button
                      ...
                      onMouseDown={e => { e.preventDefault(); handleAddUser(u); }}
                    >
          ...
          {isLoading ? (
            ...
            <div className="space-y-3">
              {people.map(person => {
                ...
                return (
                  <div key={(person as any).id || person.userId} className="flex items-center justify-between gap-3">
                    ...
                    {isOwnerRow ? (
                      ...
                      <button
                        onClick={() => handleRemoveAccess((person as any).id || person.userId, person.name)}
